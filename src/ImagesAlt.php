...
class ImagesAlt {
	public function setup() {
		...
		add_filter( 'wp_get_attachment_image_attributes', array( $this, 'add_missing_alt_attribute' ), 10, 2 );

		// Add missing alt attribute when inserting images to the editor. Work with both classic and Gutenberg editor.
		add_filter( 'wp_prepare_attachment_for_js', array( $this, 'add_missing_alt_attribute' ), 10, 2 );
	}

	public function add_missing_alt_attribute( $attr, $attachment ) {
		$attr['alt'] = $attr['alt'] ? $attr['alt'] : $attachment->post_title;
		return $attr;
	}
}
