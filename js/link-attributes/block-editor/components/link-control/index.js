...
function LinkControl ({
	...
	const handleSelectSuggestion = (suggestion, _value = {}) => {
		...
		doAction('ss-link-format-link-added', {
			...value,
			...suggestion,
			title : null
		})
	...
	const renderSearchResults = ({
		...
		const labelText = isInitialSuggestions
			? __( 'Recently updated', 'slim-seo' )
			: sprintf(
				// Translators: 1 - The search term.
				__( 'Search results for "%1$s"', 'slim-seo' ),
				inputValue
			  )
	...
	return (
		<div
			tabIndex={ -1 }
			ref={ wrapperNode }
			className="block-editor-link-control ss-link-format"
		>
			{ isResolvingLink && (
				<div className="block-editor-link-control__loading">
					<Spinner /> { __( 'Creating', 'slim-seo' ) }…
				</div>
			) }
			...
			{ value && !isEditingLink && !isResolvingLink && (
				<Fragment>
					<div
						aria-label={ __( 'Currently selected', 'slim-seo' ) }
						aria-selected="true"
			...
			<LinkControlSettingsDrawer
				key='ss-settings-drawer'
				value={ value }
				settings={ settings }
				onChange={ onChange }
				selectedText={ selectedText }
			/>
			<div id="ss-link-assistant-education">
				<div id="ss-link-assistant-education-mount"></div>
				<input
					type="hidden"
