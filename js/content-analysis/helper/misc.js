...
export const scrollToParagraph = paragraphText => {
	let selector;
	...
	if ( !isBlockEditor() ) {
		...
		selector = iframeDocument.querySelectorAll( 'p' );
	} else {
		selector = document.querySelectorAll( '.wp-block-paragraph' );
	}
	...
	const p = Array.from( selector ).find( paragraph => paragraph.textContent.includes( paragraphText ) );
