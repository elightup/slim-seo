...
const getDescriptions = () => wpDescription.value;
...
const TermDescription = ( { id, description = '', std = '', rows = 3, min = 50, max = 160, ...rest } ) => {
	...
	const descriptionRef = useRef( getDescriptions() );
	...
	const getDescription = () => sprintf( __( 'Character count: %s. Recommended length: 50-160 characters. Leave empty to autogenerate from the term description.', 'slim-seo' ), preview.length );
